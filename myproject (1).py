...
# ================== TWILIO ==================
ACCOUNT_SID = "ACa7a4c09410ff6de41309fa34359c1243"
AUTH_TOKEN  = "2c700e0be32003e5ce00b657bd06754d"
FROM_NUMBER = "+12603008332"

DRIVER_NUMBER = "+919177743848"
FAMILY_NUMBER = "+919177743848"
...
def build_twiml_driver(vehicle, location):
    """
    Driver కి వచ్చే call:
    - ముందు Telugu లో warning చెప్తుంది
    - తర్వాత English లో vehicle number + location చెప్తుంది
    - చివర మళ్ళీ Telugu లో వాహనం ఆపమని చెప్తుంది

    Google te-IN voice → Telugu text properly చదువుతుంది.
    Location numbers English లో చదివేందుకు en-IN voice వాడతాం.
    """

    # Vehicle number ని letter-by-letter readable గా చేయడం
    vehicle_readable = ", ".join(list(vehicle))

    # Location string clean చేయడం (voice కి suit అయ్యేలా)
    loc_clean = (location
...
<Response>
  ...
  <!-- Telugu లో చివర instruction -->
  <Say voice="Google.te-IN-Standard-A" language="te-IN">
    దయచేసి వాహనాన్ని వెంటనే ఆపండి. విశ్రాంతి తీసుకోండి. మీ సురక్షిత ప్రయాణం మాకు ముఖ్యం.
  </Say>
...
def build_twiml_family(vehicle, location):
    """
    Family కి వచ్చే call:
    - Telugu లో driver గురించి చెప్పడం
    - English లో vehicle number + location
    - Telugu లో సహాయం చేయమని request
    """
...
<Response>
  ...
  <!-- Telugu లో family కి alert -->
  <Say voice="Google.te-IN-Standard-A" language="te-IN">
    అత్యవసర హెచ్చరిక! మీ వాహన చోదకుడు నిద్రపోతున్నారు.
  </Say>
  <Pause length="1"/>

  <!-- English లో vehicle + location -->
  <Say voice="Polly.Raveena" language="en-IN">
  ...
  <Say voice="Polly.Raveena" language="en-IN">
    Driver's current location: {loc_clean}.
  </Say>
  <Pause length="1"/>

  <!-- Telugu లో చివర request -->
  <Say voice="Google.te-IN-Standard-A" language="te-IN">
    దయచేసి వెంటనే చోదకుడిని సంప్రదించండి లేదా సహాయం పంపండి. ధన్యవాదాలు.
  </Say>
...
def send_alert_async():
    """Driver + Family కి Telugu & English voice calls పంపడం."""
    loc = driver_location
    ...
    def _do_calls():
        ...
        if FAMILY_NUMBER != DRIVER_NUMBER:
            time.sleep(3)
            print(f"📞 Calling family: {FAMILY_NUMBER}")
            make_call(FAMILY_NUMBER, family_twiml)
        else:
            # Same number అయితే first call end అయిన తర్వాత second call
            time.sleep(35)
            print(f"📞 Re-calling same number with family message: {FAMILY_NUMBER}")
            make_call(FAMILY_NUMBER, family_twiml)
...
def generate_frames():
    ...
    while True:
        ...
        if eye_closed_frames >= THRESHOLD:
            ...
            if not alert_sent:
                alert_sent = True
                print("🚨 DROWSY ALERT — Telugu + English voice calls triggering...")
                send_alert_async()
        else:
            if status == "LIVE":
...
def update_location():
    ...
    if data:
        ...
        driver_location  = f"Lat: {lat}, Lon: {lon} (plus or minus {acc} meters)"
        driver_maps_link = f"https://maps.google.com/?q={lat},{lon}"
        print(f"📍 Location: {driver_location}")
...
<div class="container">
  ...
  <div class="side">
    ...
    <div class="card">
      <h3>🚘 వాహన సంఖ్య / Vehicle Number</h3>
      <p>{{ vehicle }}</p>
    </div>

    <div class="card">
      <h3>📊 చోదకుడి స్థితి / Driver Status</h3>
      <p id="statusText" class="status-live">LIVE</p>
      <div class="call-badge" id="callBadge">📞 Telugu + English కాల్స్ పంపబడ్డాయి!</div>
    </div>

    <div class="card">
      <h3>👁 కళ్ళు మూతపడిన frames</h3>
    ...
    <div class="card">
      <div class="info-box">
        <strong>📞 అలర్ట్ పద్ధతి:</strong><br>
        Voice Calls మాత్రమే (SMS లేదు)<br><br>
        <strong>కాల్ లో చెప్పేది:</strong><br>
        • Telugu లో నిద్ర హెచ్చరిక<br>
        • English లో vehicle number<br>
        • English లో GPS location<br>
        • Telugu లో వాహనం ఆపమని<br><br>
...
function startGPS() {
  if (!navigator.geolocation) {
    document.getElementById('locText').textContent = 'GPS support లేదు';
    return;
  }
...
function pollStatus() {
  fetch('/get_status')
    ...
    .then(d => {
      ...
      if (d.status === 'DROWSY' && prevStatus !== 'DROWSY') {
        badge.style.display = 'block';
      } else if (d.status !== 'DROWSY') {
...
if __name__ == "__main__":
    print("🚀 System started — http://localhost:5000 తెరవండి")
